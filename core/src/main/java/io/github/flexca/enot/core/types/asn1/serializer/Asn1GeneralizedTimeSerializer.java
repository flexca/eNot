...
public class Asn1GeneralizedTimeSerializer extends SimpleElementSerializer {
    ...
    protected List<ElementSerializationResult> serialize(EnotElement element, List<ElementSerializationResult> serializedBody, String jsonPath) throws EnotSerializationException {
        ...
        if (serializedBody.size() != 1) {
            throw new EnotSerializationException(EnotSerializer.COMMON_ERROR_MESSAGE, Collections.singletonList(
                    EnotJsonError.of(jsonPath, "unexpected size of arguments for " + Asn1Tag.GENERALIZED_TIME.getName()
            ...
            try {
                ...
                if (input != null) {
                    ...
                    return Collections.singletonList(ElementSerializationResult.of(Asn1EnotValueType.ASN1_ELEMENT,
                            new DERGeneralizedTime(asn1Input)));
                }

            } catch (Exception e) {
                throw new EnotSerializationException(EnotSerializer.COMMON_ERROR_MESSAGE,
                        EnotJsonError.of(jsonPath, "failure during serialization of ASN.1 element " + Asn1Tag.GENERALIZED_TIME.getName()
                                + ", reason: " + e.getMessage()), e);
            }
        }

        throw new EnotSerializationException(EnotSerializer.COMMON_ERROR_MESSAGE, Collections.singletonList(
                EnotJsonError.of(jsonPath, "unsupported body type for " + Asn1Tag.GENERALIZED_TIME.getName()
